...
From this visualisation, we can pinpoint those features with a higher variance, as they likely contribute more significantly to changes in Taylor Swift's musical style and, in turn, her receptivity.
...
It appears that `valence`, `acousticness` and `energy` have the most variance (the tails for each feature spans the widest across the graph) while `loudness` and `danceability` tie at almost equal span of values after them. These features with the widest range show notable diversity across her songs, suggesting that they play a key role in defining her sound. The large range of values in each feature could indicate large shifts in style, potentially garnering different audience responses.
When comparing among those features, we can also see that the bulge of the violin plot for `loudness` is closer to its lowest value while for `dancebility` it sits closer to the middle of its span, hence indicating that the values for `danceability` are more spread out than `loudness`. Similarly, the remaining features - `speechiness`, `liveness` and `instrumentalness` - all have values concentrated towards the left, indicating consistency across these features. Thus, we will only consider
`valence`, `acousticness`, `energy` and `danceability` as musical features to compare against Taylor Swift's receptivity due to their high span and variance in values. Moreover, a closer look at `acousticness` hints at a potential bi-modal distribution because another tiny bulge can be observed on the right side of its violin plot. Given that the values for this plot is obtained from each individual song by Swift, it could indicate that a sizeable group of her songs are purely acoustically driven.

These plots were great at hinting at potential large differences in musical features within Swift's repertoire, but they do not give us any information about how these features correlate to her receptivity. Hence, we will be plotting the mean values of `valence`, `acousticness`, `energy` and `danceability` across each album in a line graph in the next part.
...
The dual-axis plot reveals an exceptional increase in `acousticness` alongside a gradual decline in `energy`, which may correlate with album reception. This shift suggests that Taylor Swift’s evolving style has trended toward acoustic, introspective compositions, a style her audience appears to favor based on receptivity metrics. 
Interestingly, while plot 2 displays consistency in `valence` and `danceability` due to averaging feature values per album, this contrasts with the broad spread seen in plot 1 for both of them, implying that her albums likely balance happy and sad songs, together with rhythmic and flowy songs. This balance across albums may contribute to Taylor Swift’s sustained appeal.
...
#### Discussion
In "Red (Taylor's Version)", the varied distribution of features reflects the album's emotional range and dynamic appeal. `acousticness` appears to be highly polarized, echoing the depiction in plot 1, with most songs having extremely low values close to zero but a few reaching very high peaks of around 0.8 in feature strength.
This indicates a concoction of intimate and bold productions. `danceability` occupies a smaller but moderate range of values of around 0.4 to 0.7 in feature strength within this album, indicating a consistent and cohesive rhythm. 
`energy` and `valence` both span a wide range of values but their distributions are different. Within `energy` it appears that most songs are energetically similar in the upper range with smaller differences in feature strength while a small group of songs have large differences in `energy` within the lower range.
This reflects Swift's aim in diversifying this album with softer and intimate songs admist bold, powerful tunes. Lastly, `valence` appears steadily different across the songs. This ascertains our hypothesis in plot 2 that although the mean feature stength of `valence` was consistent throughout all her albums, within each album
they might spread across a large range of values. That hypothesis, also derived from the observations of the distribution of `valence` values in plot 1, is supported by this observation in plot 3. The `valence` values hint at a careful and purposeful balance in emotions ranging in this album, from happy to sad. This blend of moods
resonates with listeners who value both sad and happy songs, encapsulating the instricacies of relationships, hence enchancing the album's emotional appeal. 
