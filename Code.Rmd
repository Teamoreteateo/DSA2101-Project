...
Next, we will aggregate statistics for each of the musical attributes by taking their mean. Then, we will combine `r taylor_album_songs` and `r taylor_albums` to tie the popularity metrics with each common album.
...
Metacritic score ranges from 0 to 100 while User Score ranges from 0 to 10. We will agregate these into one statistic, "Popularity", weighted by their
