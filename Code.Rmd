...
This means that we should use data from the larger dataset to give us the widest coverage, hence use `taylor_all_songs`. 
...
```{r}
#write.csv(taylor_album_songs, "CSV files/taylor_album_songs_cleaned.csv", row.names = FALSE) This is not needed
...
Next, we will aggregate statistics for each of the musical attributes by taking their mean. Then, we will combine `taylor_all_songs` and `taylor_albums` to tie the receptivity metrics with each common album.
