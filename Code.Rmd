...
Checking the table generated, Fearless and Red does not appear in Taylor Album Songs, thus they will not be considered for popularity comparison.
Next, we will aggregate statistics for each of the musical attributes by taking their mean. Then, we will combine `r taylor_album_songs` and `r taylor_albums` tie the popularity metrics with each common album.

```{r aggregating-statistics}
taylor_album_songs %>% group_by(album_name)%>% summarize(
