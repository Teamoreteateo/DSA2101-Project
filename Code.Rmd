...
ggplot(taylor_long %>% filter(labels != "tempo"), aes(y = labels, x = values, fill = labels)) +
  geom_violin(show.legend = FALSE, alpha = 0.8, bw = 0.1, color = FALSE, width = 0.9) +
  geom_boxplot(show.legend = FALSE, alpha = 0.6, fill = "lightblue", outlier.shape = NA, width = 0.3) +
  theme_economist() +
  labs(y = "", x = "Strength (From 0 to 1)", title = "Variance of Musical Features for Swift's Songs") +
...
taylor_album_summary2 <- taylor_album_summary %>% 
                    arrange(Receptivity) %>%
                    mutate(rank = row_number()) %>%
                    pivot_longer("mean_loudness": "mean_tempo",names_to = "labels",values_to = "values") %>%
                    mutate(labels = substr(labels,6,nchar(labels)) )

ggplot(taylor_album_summary2 %>% filter(labels %in% c("acousticness", "valence", "energy", "danceability", "tempo")), aes(x = rank, y = values, color = labels)) +
  ...
  stat_smooth(se = FALSE, geom = "area",
              ...
              span = 1.3, aes(fill = labels), show.legend = FALSE) +
  geom_line(data = taylor_album_summary2,
            aes(y = Receptivity / 100,
                color = "Receptivity"),
            size = 2, linetype = "solid", lineend = "round",alpha =0.2) +
  #scale_y_continuous(
