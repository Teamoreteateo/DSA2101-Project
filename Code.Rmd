...
Although `valence` appears relatively stable in this plot (due to averaging feature values per album), Plot 3 reveals a broader range within individual songs. 
...
Both `energy` and `valence` span a wide range of values but their distributions are different. Within `energy` it appears that most songs are energetically similar in the upper range with smaller differences in feature strength while a small group of songs have large differences in `energy` within the lower range, creatning a rich emotional mix.
