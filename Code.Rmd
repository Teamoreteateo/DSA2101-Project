...
ggplot(taylor_long %>% filter(labels != "tempo"), aes(y = labels, x = values, fill = labels)) +
  ...
  theme_economist() +
  labs(y = "", x = "Feature Strength (From 0 to 1)", title = "Variance of Musical Features for Swift's Songs") +
...
ggplot(taylor_album_plot %>% filter(labels %in% c("acousticness", "valence", "energy", "danceability", "tempo")), aes(x = rank, y = values, color = labels)) +
  ...
  stat_smooth(se = FALSE, geom = "area",
              ...
              span = 1.2, aes(fill = labels), show.legend = F) +
...
ggplot(scaled_songs_long, aes(y = value, x = reorder_within(track_number, value, feature), fill = feature)) +
  geom_col() +
